%% plotting_net_benefit.m
% 
% Creates a plot comparing the output equivalent net benefit from reneging
% for simulations with and without financial frictions.
% 
% Based on plotFigureC2.m from the original authors.
% 
...
% Plot net benefit
plot(qtr_dates,SimOutput_noFin.EXANTE.netBenefit(1,:),\'b-\
     qtr_dates,SimOutput_Fin.EXANTE.netBenefit(1,:),\'r--',
     'LineWidth',line_width);

hold on;

% Add zero line
plot(qtr_dates,zeros(size(qtr_dates)),\'k:\');

% Add labels and title
xlabel('Year','FontSize',font_size);
ylabel('Percent of quarterly output','FontSize',font_size);
title('Ex Ante Output Equivalent Net Benefit from Reneging','FontSize',font_size);
...
% Save the figure
fig_name = \'compare_net_benefit.pdf\';

...
while (isELBviolatedOnExit||epsrMaxChange>epsrChangeTol) && iter<maxIter
    ...
    if ~isSolutionAdmissable
        fprintf(['\t\tThe quadprog routine failed to find the ',...
            'solution in one or more lift-off periods\n']);
    end
    epsrPaths = bifurcationFrac*epsrPathsNew+(1-bifurcationFrac)*epsrPaths;
    xPaths = compute_simulation_paths(...
        JJ,HtildeBxtilde,HtildeBr,HtildeCr,HtildeFxtilde,HtildeFr,...
        PSItildeztilde,HhatBxtilde,HhatBr,HhatCxtilde,HhatCr,...
        HhatFxtilde,HhatFr,PSIhatepsr,B,Bxtildextilde,Bxtilder,Brxtilde,...
        Brr,F,PHIepsr,xtilde0,r0,ztilde,epsrPaths,ZZtilde,Sxtilde,Sr,...
    ...
    if ~any(any(rPaths<b-bViolationTol))
        ...
        if displayProgress
            fprintf(['\t\tMax violation of ELB: ',...
                num2str(maxViolation),'\n']);
        end
    end
    rMaxChange = max(max(abs(rPaths-rPathsOld)));
    if displayProgress
        fprintf(['\t\tMax change in rates is: ',num2str(rMaxChange),'\n']);
    end
    epsrMaxChange = max(max(abs(epsrPathsNew-epsrPathsOld)));
end
...
for i = 1:K+1
    fquad = rPathsNoPolicyShocks(1,i:H,i)'-b;
    iaPolicyShockMultMat = ...
        aPolicyShockMultMatForQuadProg(i:H,i:H,i);
    lbquad = zeros(H-i+1,1);
    x0 = zeros(H-i+1,1);
    Hquad = (iaPolicyShockMultMat+iaPolicyShockMultMat');
    Aquad = -iaPolicyShockMultMat;
    bquad = fquad';
    [x,fval,iQuadProgWorked] = quadprog(...
        Hquad,fquad,Aquad,bquad,[],[],lbquad,[],x0,options);
    if iQuadProgWorked ~= 1
        error(['Quadratic programming problem to find policy ',...
            'shocks to enforce occassionally binding instrument constraint ',...
            'failed for exit in period ',num2str(i)]);
    end
    if abs(fval) > 1e-6

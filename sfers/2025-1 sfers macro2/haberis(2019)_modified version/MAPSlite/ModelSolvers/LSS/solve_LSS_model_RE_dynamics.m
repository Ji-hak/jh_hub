...
% NOTE THAT THIS VERSION HAS BEEN **HACKED** TO ENSURE THAT MODELS WITH
% PURE UNIT ROOTS CAN BE SOLVED
...
%% SET TOLERANCE LEVEL FOR UNIT ROOTS
smallNumber = 1e-12;
...
if any(any(HF))
    [B,~,~,~,~,~,aimcode] = SPAmalg([HB HC HF],nx,1,1,1e-10,1+smallNumber);
    if aimcode ~= 1
       [B,~,~,~,~,~,aimcode] = ...
           SPAmalg([HB HC HF],nx,1,1,1e-10,1);
    end
    if aimcode ~= 1
